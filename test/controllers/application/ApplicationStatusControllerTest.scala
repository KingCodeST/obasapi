...
class ApplicationStatusControllerTest extends PlaySpec with GuiceOneAppPerTest  with Injecting {
  ...
  val entity = ApplicationStatus("1","to be deleted","",Some("Recieved") , LocalDateTime.now())
  ...
  "EntityController" should {
    ...
    "Read Entity " in {
      val request = route(app, FakeRequest(GET, "/application/status/get/$applicationStatusId" + entity.statusId)
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
